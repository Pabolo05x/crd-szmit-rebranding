...
// 5. LOGIKA WYBORU KURSU (ZAKTUALIZOWANA - NAPRAWIA PROBLEM MAPY)
function prefillMessage(courseValue) {
    // Zapisujemy wybrany kurs w pamięci
    localStorage.setItem('selectedCourse', courseValue);
    // Przekierowanie do konkretnego ID formularza (NIE do ogólnego #kontakt)
    window.location.href = 'index.html#formularz-kontaktowy';
}
...
document.addEventListener('DOMContentLoaded', () => {
    ...
    if (savedCourse && courseSelect) {
        ...
        // Przewijamy do formularza (nie do sekcji kontakt)
        setTimeout(() => {
            const formSection = document.getElementById('formularz-kontaktowy');
            if (formSection) {
                const headerOffset = 120; // Margines na menu
        ...
        }, 500); // Opóźnienie 500ms, aby ominąć ładowanie mapy
    }
});
...
// 7. FIX: INTELIGENTNE PRZEWIJANIE DO FORMULARZA NA MOBILE (Dla linków z menu)
document.addEventListener("DOMContentLoaded", function() {
    if (window.location.hash === "#formularz-kontaktowy") {
        setTimeout(function() {
            const element = document.getElementById("formularz-kontaktowy");
            if (element) {
